                ...
                if (m.get(x,y).team != 0)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                mbuf.setLife(x + dx, y + dy,
                                    m.get(x + dx, y + dy).life +
                                    max((m.get(x, y).life -
                                        m.get(x + dx, y + dy).life)*0.2, 0.0));
                            }
                            else
                            {
                                mbuf.setLife(x, y, m.get(x, y).life -
                                    max(m.get(x + dx, y + dy).force -
                                        m.get(x, y).defence, 0.0));

                                if (mbuf.get(x, y).life <= 20)
                                {
                                    mbuf.set(x, y, Warrior(0, 0, 0, 0));
                                    mbuf.setForce(x + dx, y + dy,
                                        m.get(x + dx, y + dy).force + 0.4);
                                }
                                else
                                    mbuf.setDefence(x, y, m.get(x, y).defence + 0.0001);

                                mbuf.setLife(x + dx, y + dy,
                                    m.get(x + dx, y + dy).life -
                                    max(m.get(x, y).force -
                                        m.get(x + dx, y + dy).defence, 0.0));
                                ...
                                if (mbuf.get(x + dx, y + dy).life <= 20)
                                {
                                    ...
                                    mbuf.setDefence(x + dx, y + dy,
                                        m.get(x + dx, y + dy).defence + 0.0001);
                            }
                        }
                    }
                }
...
void Board::draw(sf::RenderWindow &window)
{
    for (int x(0); x < width; x++)
        for (int y(0); y < height; y++)
            image.setPixel(x, y,
                HSVColor(m.get(x,y).team * 89,
                    min(m.get(x, y).force*0.8,100.0),
                    m.get(x, y).life * 1).RGB());
